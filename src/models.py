...
from diffusers.pipelines.qwenimage import QwenImageInpaintPipeline
...
class ModelManager:
    ...
    def __init__(self):
        ...
        self.pipe_inpaint = None  # Separate inpainting pipeline
    ...
    def build_inpaint_pipeline(self, force_rebuild: bool = False) -> QwenImageInpaintPipeline:
        """Build the inpainting pipeline from components.
        
        Args:
            force_rebuild: Force rebuild even if already built
            
        Returns:
            Assembled inpainting pipeline
        """
        if self.pipe_inpaint and not force_rebuild:
        ...
        config = get_config()
        print("Building inpainting pipeline from components")
        
        # Use the actual QwenImageInpaintPipeline from diffusers
        self.pipe_inpaint = QwenImageInpaintPipeline(
            text_encoder=self.text_encoder,
            tokenizer=self.tokenizer,
            transformer=self.transformer,
            vae=self.vae,
        ...
        self.component_status['pipeline_inpaint'] = True
        print("Inpainting pipeline assembled")
    ...
    def unload_component(self, component: str):
        ...
        elif component == 'pipeline_inpaint' and self.pipe_inpaint:
            self.pipe_inpaint = None
            self.component_status['pipeline_inpaint'] = False
    ...
    def unload_all(self):
        ...
        for component in ['pipeline', 'pipeline_img2img', 'pipeline_inpaint', 'transformer', 'vae', 'scheduler', 
...
def get_inpaint_pipe() -> QwenImageInpaintPipeline:
    """Get the inpainting pipeline, loading if necessary."""
    manager = get_model_manager()
    return manager.build_inpaint_pipeline()
...
def get_pipeline(mode: str = "txt2img") -> Any:
    ...
    Args:
        mode: Generation mode ("txt2img", "img2img", or "inpaint")
